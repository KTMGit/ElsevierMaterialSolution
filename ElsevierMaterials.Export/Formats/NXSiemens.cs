...
namespace ElsevierMaterials.Exporter.Formats
{
    ...
    public class NXSiemens
    {
        ...
        private static void FillPropertyData(XElement BulkDetails, IList<ExportPropertyGeneral> properties)
        {
            ...
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.PhysicalDensity select u).FirstOrDefault();
            ...
            //Youngs modul
            if ((from u in properties where u.Type == TMPropertyTypeEnum.PhysicalModulusOfElasticity  select u).Any())
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.PhysicalModulusOfElasticity select u).FirstOrDefault();
                value = (double.Parse(property.Value) * 1000000).ToString("e6", System.Globalization.CultureInfo.InvariantCulture);
                CreatePropertyData(BulkDetails, value, "exponential", "Youngs Modulus (E)_31");
            }
            //Poisson ratio
            if ((from u in properties where u.Type == TMPropertyTypeEnum.PhysicalPoissonCoefficient  select u).Any())
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.PhysicalPoissonCoefficient select u).FirstOrDefault();
            ...
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.MechanicalYield select u).FirstOrDefault();
            ...
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.MechanicalTensile select u).FirstOrDefault();
                value = (double.Parse(property.Value) * 1000).ToString("e6", System.Globalization.CultureInfo.InvariantCulture);
                CreatePropertyData(BulkDetails, value, "exponential", "Ultimate Tensile Strength_33");
            }
            //Fatigue Strength Coefficient_20
            if ((from u in properties where u.Type == TMPropertyTypeEnum.FatigueStrengthCoefficient  select u).Any())
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.FatigueStrengthCoefficient select u).FirstOrDefault();
                value = (double.Parse(property.Value) * 1000).ToString("e6", System.Globalization.CultureInfo.InvariantCulture);
                CreatePropertyData(BulkDetails, value, "exponential", "Fatigue Strength Coefficient_20");
            }
            //FatigueStrengthExp
            if ((from u in properties where u.Type == TMPropertyTypeEnum.FatigueStrengthExponent  select u).Any())
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.FatigueStrengthExponent select u).FirstOrDefault();
                value = (double.Parse(property.Value)).ToString("e6", System.Globalization.CultureInfo.InvariantCulture);
                CreatePropertyData(BulkDetails, value, "exponential", "FatigueStrengthExp");
            }
            //FatigueDuctCoeff
            if ((from u in properties where u.Type == TMPropertyTypeEnum.FatigueDuctilityCoefficient  select u).Any())
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.FatigueDuctilityCoefficient select u).FirstOrDefault();

                value = (double.Parse(property.Value)).ToString("e6", System.Globalization.CultureInfo.InvariantCulture);
                CreatePropertyData(BulkDetails, value, "exponential", "FatigueDuctCoeff");
            }
            //FatigueDuctExp
            if ((from u in properties where u.Type == TMPropertyTypeEnum.FatigueDuctilityExponent  select u).Any())
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.FatigueDuctilityExponent select u).FirstOrDefault();
                value = (double.Parse(property.Value)).ToString("e6", System.Globalization.CultureInfo.InvariantCulture);
                CreatePropertyData(BulkDetails, value, "exponential", "FatigueDuctExp");
            }
            //CyclicStrengthCoeff
            if ((from u in properties where u.Type == TMPropertyTypeEnum.FatigueCyclicStrengthExponent  select u).Any())
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.FatigueCyclicStrengthExponent select u).FirstOrDefault();
                value = (double.Parse(property.Value) * 1000).ToString("e6", System.Globalization.CultureInfo.InvariantCulture);
            ...
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.PhysicalMeanCoeffThermalExpansion select u).FirstOrDefault();
                value = (double.Parse(property.Value) / 1000000).ToString("e6", System.Globalization.CultureInfo.InvariantCulture);
            ...
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.PhysicalThermalConductivity select u).FirstOrDefault();
                value = (double.Parse(property.Value) * 1000).ToString("e6", System.Globalization.CultureInfo.InvariantCulture);
            ...
            {
                property = (from u in properties where u.Type == TMPropertyTypeEnum.PhysicalSpecificThermalCapacity select u).FirstOrDefault();
                value = (double.Parse(property.Value) * 1000000).ToString("e6", System.Globalization.CultureInfo.InvariantCulture);
        ...
        private static IList<NXSiemensParameterDetail> FillListOFParameterDetails()
        {
            ...
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.MechanicalYield, "Yield Strength_32", "Yield", new List<NXSiemensUnit> { new NXSiemensUnit() { Name = "mN/mm^2(kPa)" } }, "mN/mm^2(kPa)", "milliNewtons per millimeter squared");
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.MechanicalTensile, "Ultimate Tensile Strength_33","UltTensile", new List<NXSiemensUnit> { new NXSiemensUnit() { Name = "mN/mm^2(kPa)" } }, "mN/mm^2(kPa)", "milliNewtons per millimeter squared");
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.MechanicalTensile, "Thermal Expansion (A)_34", "ThermalExpansion", new List<NXSiemensUnit> { new NXSiemensUnit() { Name = "1/C" } }, "1/C", "Expansion coefficient per degree Celsius");
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalThermalConductivity, "Thermal Conductivity (K)_35", "ThermalConductivity", new List<NXSiemensUnit> { new NXSiemensUnit() { Name = "microW/mm-C" } }, "microW/mm-C", "microWatts per millimeter per degree Centrigade");
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "Youngs Modulus (E)_31", "YoungsModulus", new List<NXSiemensUnit> { new NXSiemensUnit() { Name = "mN/mm^2(kPa)" } }, "mN/mm^2(kPa)", "milliNewtons per millimeter squared");
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "Specific Heat (CP)_23", "SpecificHeat", new List<NXSiemensUnit> { new NXSiemensUnit() { Name = "microJ/kg-K" } }, "microJ/kg-K", "microJoules per kilogram per degree Kelvin");
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "Mass Density (RHO)_6", "MassDensity", new List<NXSiemensUnit> { new NXSiemensUnit() { Name = "kg/mm^3" } }, "kg/mm^3", "kilograms per millimeter cubed");
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "Material Type", "Material Type", new List<NXSiemensUnit> {null}, null, null);
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "Version", "Version", new List<NXSiemensUnit> { null }, null, null);
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "Category", "Category", new List<NXSiemensUnit> { null }, null, null);
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "PoissonsRatio", "PoissonsRatio", new List<NXSiemensUnit> { null }, null, null);
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "FatigueStrengthExp", "FatigueStrengthExp", new List<NXSiemensUnit> { null }, null, null);
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "FatigueDuctCoeff", "FatigueDuctCoeff", new List<NXSiemensUnit> { null }, null, null);
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "FatigueDuctExp", "FatigueDuctExp", new List<NXSiemensUnit> { null }, null, null);
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "CyclicStrainHardeningExp", "CyclicStrainHardeningExp", new List<NXSiemensUnit> { null }, null, null);
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "Fatigue Strength Coefficient_20", "FatigueStrengthCoeff", new List<NXSiemensUnit> { new NXSiemensUnit() { Name = "mN/mm^2(kPa)" } }, "mN/mm^2(kPa)", "milliNewtons per millimeter squared");
            listOfParameters.Add(parameterDetail);
            parameterDetail = CreateParameterDetail(TMPropertyTypeEnum.PhysicalModulusOfElasticity, "CyclicStrengthCoeff", "CyclicStrengthCoeff", new List<NXSiemensUnit> { new NXSiemensUnit() {  Name = "mN/mm^2(kPa)" } }, "mN/mm^2(kPa)", "milliNewtons per millimeter squared");
            listOfParameters.Add(parameterDetail);
        ...
        /// <returns></returns>
        private static NXSiemensParameterDetail CreateParameterDetail(TMPropertyTypeEnum type, string idPrefix,  string Name, IList<NXSiemensUnit> units, string unitsName, string unitsDescription)
        {
