...
namespace ElsevierMaterials.Exporter.Formats
{
    public class Abaqus
    {
        ...
        {           
            ...
            exportString = exportString + FormatValue(properties, TMPropertyTypeEnum.PhysicalModulusOfElasticity) + ", " + FormatValue(properties, TMPropertyTypeEnum.PhysicalPoissonCoefficient) + System.Environment.NewLine;
            ...
            ExportProperty property = (from u in properties where u.Type == TMPropertyTypeEnum.PlasticStrainStress select u).FirstOrDefault() as ExportProperty;
            if (property != null)
            {
                ...
                exportString = exportString + FormatValue(properties, TMPropertyTypeEnum.PlasticStrainStress);
            }
            
            exportString = exportString + DENSITY + System.Environment.NewLine;
            exportString = exportString + FormatValue(properties, TMPropertyTypeEnum.PhysicalDensity) + System.Environment.NewLine;
            return exportString;
        }
        /// <summary>
        ...
        private static string FormatThermalConductivity(IList<ExportPropertyGeneral> adddedProperties)
        {
            ...
            List<ExportPropertyGeneral> properties = (from u in adddedProperties where u.Type == TMPropertyTypeEnum.PhysicalThermalConductivity select u).ToList();
        ...
        private static string FormatThermalExpansion(IList<ExportPropertyGeneral> adddedProperties)
        {
            ...
            List<ExportPropertyGeneral> properties = (from u in adddedProperties where u.Type == TMPropertyTypeEnum.PhysicalMeanCoeffThermalExpansion select u).ToList();
        ...
        private static string FormatSpecificThermalCapacity(IList<ExportPropertyGeneral> adddedProperties)
        {
            ...
            List<ExportPropertyGeneral> properties = (from u in adddedProperties where u.Type == TMPropertyTypeEnum.PhysicalSpecificThermalCapacity select u).ToList();
            string value = ZERO;
            string temperature = LEFTZERO;
            string text = value + ", \t" + temperature + System.Environment.NewLine;
            if (properties.Count > 0)
        ...
        /// <returns></returns>
        private static string FormatValue(IList<ExportPropertyGeneral> properties, TMPropertyTypeEnum ExportType, StressStrainType subtype = StressStrainType.None)
        {
            ...
            if (property != null)
            {
             ...
             switch (ExportType)
             {
                 ...
                 case TMPropertyTypeEnum.PhysicalDensity:
                     ...
                     return value;
                 case TMPropertyTypeEnum.PlasticStrainStress:
                     if (property!=null)
                     {
                         string table = "";
