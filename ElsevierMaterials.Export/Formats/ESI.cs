    ...
    public class ESI
    {
        ...
        private static void AddStressStrainNew(IList<Models.ExportPropertyGeneral> properties, XElement Model)
        {
            if (properties.Where(p => p.Type == TMPropertyTypeEnum.StressStrain).Any())
            {
                ...
                coordinates = properties.Where(p => p.Type == TMPropertyTypeEnum.StressStrain).Select(u => u.Values).FirstOrDefault().ToList();
        ...
        {
            if (properties.Where(p => p.Type == TMPropertyTypeEnum.MechanicalElongation || p.Type == TMPropertyTypeEnum.MechanicalTensile || p.Type == TMPropertyTypeEnum.MechanicalYield || p.Type == TMPropertyTypeEnum.StressStrain).Any())
            {
                XElement Model = new XElement("Model");
        ...
        {
            if (properties.Where(p => p.Type == TMPropertyTypeEnum.PhysicalModulusOfElasticity || p.Type == TMPropertyTypeEnum.PhysicalMeanCoeffThermalExpansion).Any())
            {
                XElement Model = new XElement("Model");
        ...
        {            
            if (properties.Where(p => p.Type == TMPropertyTypeEnum.StressStrain).Any())
            {
                ...
                IList<ExportPropertyGeneral> proeprties = properties.Where(p => p.Type == TMPropertyTypeEnum.StressStrain).ToList();
